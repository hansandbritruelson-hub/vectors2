...
struct Args {
    ...
    #[arg(short, long, help = "Path to the output structure (usually generated_ui/app.rs)")]
    output: PathBuf,
}
...
fn compile_recursive(input_path: &Path, output_path: &Path, visited: &mut HashSet<PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file_stem = output_path.file_stem().and_then(|s| s.to_str()).unwrap_or("app");
    ...
    for import in &template.imports {
        if import.is_component {
            ...
            if child_input.exists() {
                ...
                println!("Warning: Component {} imports module '{}' but {}.vue not found.", input_path.display(), import.module_name, import.module_name);
            }
        }
    }
    ...
    // Run rustfmt
    let format_status = Command::new("rustfmt")
        .arg(output_path)
        .status();
