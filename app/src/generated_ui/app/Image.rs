...
fn estimate_cubic_steps(p0: (f32, f32), p1: (f32, f32), p2: (f32, f32), p3: (f32, f32)) -> usize {
    ...
    let control_net = distance(p0.0, p0.1, p1.0, p1.1)
...
fn build_bezier_overlay_paths(objects: &[VectorObject]) -> BezierOverlayPaths {
    ...
    for object in objects {
        ...
        for point in &object.points {
            append_square_marker_path(
                &mut overlay.anchor_marker_path_data,
                point.x,
                point.y,
                ANCHOR_SIZE,
            );
            if let Some(handle_in) = &point.handle_in {
                ...
                append_square_marker_path(
                    &mut overlay.handle_marker_path_data,
                    handle_in.x,
                    handle_in.y,
                    HANDLE_SIZE,
                );
...
pub fn build_with_slots(
    ...
    let (curve_overlay_path, set_curve_overlay_path) =
        crate::signals::create_signal::<String>(initial_overlay.curve_path_data);
    let (guide_overlay_path, set_guide_overlay_path) =
        crate::signals::create_signal::<String>(initial_overlay.guide_path_data);
    let (anchor_marker_path, set_anchor_marker_path) =
        crate::signals::create_signal::<String>(initial_overlay.anchor_marker_path_data);
    let (handle_marker_path, set_handle_marker_path) =
        crate::signals::create_signal::<String>(initial_overlay.handle_marker_path_data);
    ...
    let on_canvas_mouse_down = Rc::new(move |event: renderer_core::UiEvent| {
        ...
        let (canvas_x, canvas_y) = engine_ref_down
            .borrow()
    ...
    let on_canvas_mouse_move = Rc::new(move |event: renderer_core::UiEvent| {
        ...
        let (canvas_x, canvas_y) = engine_ref_move
            .borrow()
            .get_node_final_position(event.current_target.id);
        let x = event.mouse_x - canvas_x;
        let y = event.mouse_y - canvas_y;
        if pen_drag_point_move.borrow().is_none() {
            ...
            match insert_pen_point(
                &mut next_objects,
                &current_path_id,
                down_x,
                down_y,
                snapping_enabled.get(),
            ) {
                PenInsertResult::NewPathStarted {
                    path_id,
                    point_index,
                } => {
                    let _ = set_pen_point_mirrored_handles(
                        &mut next_objects,
                        &path_id,
                        point_index,
                        x,
                    ...
                    *pen_drag_point_move.borrow_mut() = Some(PenDragPoint {
                        path_id,
                        point_index,
                    });
                }
        ...
        let mut next_objects = objects.get();
        if set_pen_point_mirrored_handles(
            &mut next_objects,
            &drag_state.path_id,
    ...
    let on_canvas_mouse_up = Rc::new(move |event: renderer_core::UiEvent| {
        ...
        let (canvas_x, canvas_y) = engine_ref_up
            .borrow()
            .get_node_final_position(event.current_target.id);
        let x = event.mouse_x - canvas_x;
        let y = event.mouse_y - canvas_y;
                ...
                .build(engine.clone(), Some(node_1));
            {
                    ...
                    .build(engine.clone(), Some(node_30));
                {
                        ...
                        .build(engine.clone(), Some(node_66));
                    {
                        let node_68 = div()
                            .class("canvas-mock")
                            .on_mouse_down(move |event: renderer_core::UiEvent| {
                                (on_canvas_mouse_down)(event);
                            })
                            .on_mouse_move(move |event: renderer_core::UiEvent| {
                                (on_canvas_mouse_move)(event);
                            })
                            .on_mouse_up(move |event: renderer_core::UiEvent| {
                                (on_canvas_mouse_up)(event);
                            })
                            .build(engine.clone(), Some(node_67));
                        {
                            ...
                            {
                                let node_70 = div()
                                    .class("bezier-guide-overlay")
                                    .bind_path(create_memo({
                                        let val = guide_overlay_path.clone().clone();
                                        move || val.to_reactive_string()
                                    }))
                                    .build(engine.clone(), Some(node_69));
                                {}
                                node_70;
                                let node_71 = div()
                                    .class("bezier-curve-overlay")
                                    .bind_path(create_memo({
                                        let val = curve_overlay_path.clone().clone();
                                        move || val.to_reactive_string()
                                    }))
                                    .build(engine.clone(), Some(node_69));
                                {}
                                node_71;
                                let node_72 = div()
                                    .class("bezier-handle-marker-overlay")
                                    .bind_path(create_memo({
                                        let val = handle_marker_path.clone().clone();
                                        move || val.to_reactive_string()
                                    }))
                                    .build(engine.clone(), Some(node_69));
                                {}
                                node_72;
                                let node_73 = div()
                                    .class("bezier-anchor-marker-overlay")
                                    .bind_path(create_memo({
                                        let val = anchor_marker_path.clone().clone();
                                        move || val.to_reactive_string()
                                    }))
                                    .build(engine.clone(), Some(node_69));
                                {}
                        ...
                        .build(engine.clone(), Some(node_66));
                    {
                        let node_75 = div()
                            .class("panel-header")
                            .build(engine.clone(), Some(node_74));
                        {
                            let node_76 = img()
                                .image("asset://phosphor/layers.svg")
                                .class("icon-img-mini")
                                .build(engine.clone(), Some(node_75));
                            {}
                            node_76;
                            let node_77 = div()
                                .class("panel-title")
                                .build(engine.clone(), Some(node_75));
                            {
                                div().text("Objects").build(engine.clone(), Some(node_77));
                            }
                            node_77;
                        }
                        node_75;
...
fn register_styles(engine: Rc<RefCell<FlexEngine>>) {
    ...
    }
    {
        ...
        decls.insert(
            "fill".to_string(),
            renderer_core::StyleValue::Color(0.0627451f32, 0.5686275f32, 0.9647059f32, 0.55f32),
        );
        decls.insert(
            "stroke".to_string(),
            renderer_core::StyleValue::Color(0f32, 0f32, 0f32, 0f32),
        ...
        e.add_style_rule(".bezier-guide-overlay".to_string(), decls);
    }
    {
        let mut decls = std::collections::HashMap::new();
        decls.insert(
            "fill".to_string(),
            renderer_core::StyleValue::Color(1f32, 0.3529412f32, 0.16470589f32, 1f32),
        );
        decls.insert("top".to_string(), renderer_core::StyleValue::Px(0f32));
        decls.insert(
            "width".to_string(),
        ...
        decls.insert("left".to_string(), renderer_core::StyleValue::Px(0f32));
        e.add_style_rule(".bezier-curve-overlay".to_string(), decls);
    }
    {
        let mut decls = std::collections::HashMap::new();
        decls.insert(
            "fill".to_string(),
            renderer_core::StyleValue::Color(0.0627451f32, 0.5686275f32, 0.9647059f32, 1f32),
        );
        decls.insert(
            "stroke".to_string(),
            renderer_core::StyleValue::Color(0f32, 0f32, 0f32, 0f32),
        ...
        e.add_style_rule(".bezier-handle-marker-overlay".to_string(), decls);
    }
    {
        let mut decls = std::collections::HashMap::new();
        decls.insert(
        ...
        e.add_style_rule(".bezier-anchor-marker-overlay".to_string(), decls);
    }
    {
        let mut decls = std::collections::HashMap::new();
        decls.insert(
