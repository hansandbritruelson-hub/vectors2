...
fn main() {
    ...
    let toml_str = fs::read_to_string(&toml_path)
        .expect("Failed to read style_defs.toml");
    let toml_val: toml::Value = toml_str.parse()
        .expect("Failed to parse style_defs.toml");
    ...
    for (name, id) in &properties {
        writeln!(rust_file, "pub const PROP_{}: u32 = {};", name.to_uppercase(), id).unwrap();
    }

    writeln!(rust_file, "").unwrap();
    writeln!(rust_file, "// --- Control Markers ---").unwrap();
    for (name, id) in &controls {
        writeln!(rust_file, "pub const CTRL_{}: u32 = {};", name.to_uppercase(), id).unwrap();
    }

    writeln!(rust_file, "").unwrap();
    writeln!(rust_file, "// --- Unit IDs ---").unwrap();
    for (name, id) in &units {
        writeln!(rust_file, "pub const UNIT_{}: u32 = {};", name.to_uppercase(), id).unwrap();
    }
    ...
    for (name, id) in &properties {
        writeln!(wgsl_file, "const PROP_{}: u32 = {}u;", name.to_uppercase(), id).unwrap();
    }

    writeln!(wgsl_file, "").unwrap();
    writeln!(wgsl_file, "// --- Control Markers ---").unwrap();
    for (name, id) in &controls {
        writeln!(wgsl_file, "const CTRL_{}: u32 = {}u;", name.to_uppercase(), id).unwrap();
    }

    writeln!(wgsl_file, "").unwrap();
    writeln!(wgsl_file, "// --- Unit IDs ---").unwrap();
    for (name, id) in &units {
        writeln!(wgsl_file, "const UNIT_{}: u32 = {}u;", name.to_uppercase(), id).unwrap();
    }
}
